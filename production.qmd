...
To ensure compatibility, please use R version `r snapshot_r()` and [CRAN](https://cran.r-project.org/) dependencies
from `r snapshot_date()`. [Posit Public Package Manager (p3m)](https://packagemanager.posit.co) can help. Example:^[Caveat: when setting two URLs in the repos argument, the same package may be in both repositories, and the highest version is always preferred.]
