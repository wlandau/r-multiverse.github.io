...
In addition, it must pass the following list of automated checks in the [Staging repository](#staging) at least once during the current [candidate freeze](#candidate-freeze):
...
* All strong dependencies^[A strong dependency is another package listed in the `Depends:`, `Imports:`, or `LinkingTo:` field of the `DESCRIPTION` file.] from R-multiverse^[An R-multiverse package can strongly depend on a package from CRAN, regardless of CRAN check status, as long as the CRAN package exists in the Posit Public Package manager snapshot used in [Staging](#staging).] must also qualify^[The dependency must pass at least once during the current [candidate freeze](#candidate-freeze), but not necessarily at the same time as the downstream packages that depend on it.] for Production.
